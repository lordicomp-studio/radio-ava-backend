    ...
    <section class="tableData itemIndex bg-white p-6 rounded">
        <header class="grid grid-cols-[auto,auto] justify-between">
            ...
            <div class="left flex items-center">  <!-- space-x-2 space-x-reverse-->
                <inertia-link href="/admin/pages/create" class="btn btn-primary btn-active-primary !m-0">
                    <SvgComponent class="transition ease-in-out duration-150" name="add-simple" size="1.1rem" color="#ccc"/>
                    {{ $t('tables.common.add') }} {{ $t('pages.Pages') }}
                </inertia-link>
            </div>
        </header>
        <main class="pt-6">
            <ul class="relative">
                ...
                <li class="contents" v-for="(item,index) in store.tableData.data" :key="index"
                    ...
                    <div class="item grid grid-cols-[auto,auto] justify-start gap-2">
                        <span>{{item.name}}</span>
                    </div>

                    <TableUserItem :user="item.creator" profilePhotoFieldName="profile_photo"/>
...
import {usePagesStore} from "../../../Stores/admin/Pages/IndexStore";
