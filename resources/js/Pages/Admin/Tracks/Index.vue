            ...
            <ul class="relative">
                ...
                <li class="contents" v-for="(item,index) in store.tableData.data" :key="index"
                    ...
                    <figure class="h-[50px] w-[50px] rounded-full overflow-hidden">
                        <img
                            :src="item.cover ? `/storage${item.cover.url}` : '/Images/image not found.webp'"
                    ...
                    <TablePicturedItem :pictureUrl="item.artist?.photo?.url" link=""
                                       :text="item.artist ? item.artist.name : 'ناموجود'" />

                    <span>{{ item.album ? item.album.name : 'سینگل' }}</span>

                    <TableUserItem :user="item.uploader" profilePhotoFieldName="profile_photo"/>
...
import {useTracksStore} from "../../../Stores/admin/Tracks/IndexStore";
